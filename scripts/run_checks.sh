...
ls /tmp/kagent-wheelhouse/kagent-0.1.6-*.whl >/dev/null
PYTHONWARNINGS=ignore .venv/bin/kagent-release-manifest \
    /tmp/kagent-wheelhouse/kagent-0.1.6-*.whl \
...
printf '{"package":"kagent","version":"0.1.6","artifact_count":"1","artifacts":[{"sha256":"abc","size_bytes":"123"}]}\n' >/tmp/kagent-release-manifest-missing-path.json
if PYTHONWARNINGS=ignore .venv/bin/kagent-release-manifest \
...
printf '{"package":"kagent","version":"0.1.6","artifact_count":"1","artifacts":[{"path":"/tmp/kagent-release-manifest-artifact-dir","sha256":"abc","size_bytes":"123"}]}\n' >/tmp/kagent-release-manifest-directory-path.json
if PYTHONWARNINGS=ignore .venv/bin/kagent-release-manifest \
...
ls /tmp/kagent-isolated-wheelhouse/kagent-0.1.6-*.whl >/dev/null
rm -rf /tmp/kagent-wheel-install-venv
PYTHONWARNINGS=ignore .venv/bin/python -m venv /tmp/kagent-wheel-install-venv
PYTHONWARNINGS=ignore /tmp/kagent-wheel-install-venv/bin/python -m pip install --no-deps /tmp/kagent-wheelhouse/kagent-0.1.6-*.whl >/tmp/kagent-wheel-install.log
