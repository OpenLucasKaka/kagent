...
`kagent-release-evidence` is the handoff artifact for internal
...
`kagent` and scrapes `/metrics.prom` on the `http` port with the
...
`kagent-release-evidence` so the final approval bundle is also
...
`KAGENT_OBSERVABILITY_BASE_URL` and a short-lived
`KAGENT_OBSERVABILITY_TOKEN` when diagnostics are protected. Add
`KAGENT_PROMETHEUS_BASE_URL` and, if needed,
`KAGENT_PROMETHEUS_TOKEN` to prove Prometheus can query
`KAGENT_PROMETHEUS_QUERY` with at least one result. Attach that
...
`KAGENT_PROVIDER_SMOKE_EVIDENCE`,
`KAGENT_STAGING_ACCEPTANCE_EVIDENCE`,
`KAGENT_OBSERVABILITY_ACCEPTANCE_EVIDENCE`,
`KAGENT_INTERNAL_ROLLOUT_EVIDENCE`,
`KAGENT_RELEASE_MANIFEST`, `KAGENT_RUN_CHECKS_EXIT_CODE`,
`KAGENT_READINESS_AUDIT_OUTPUT`, or
`KAGENT_RELEASE_EVIDENCE_OUTPUT` when release automation stores
...
`KAGENT_EVIDENCE_MAX_AGE_SECONDS` only when the release process has an
...
- Subject-level runtime tool execution policy via
  `KAGENT_SERVICE_RUNTIME_ALLOWED_TOOLS_BY_SUBJECT` for internal
...
- `deploy/kubernetes/kagent.yaml` for Kubernetes deployment with
