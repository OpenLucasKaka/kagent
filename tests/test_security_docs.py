...
def test_security_policy_documents_reporting_and_runtime_controls():
    ...
    assert "KAGENT_SERVICE_AUTH_TOKEN" in security
    ...
    assert "KAGENT_SERVICE_RATE_LIMIT_PER_MINUTE" in security
    assert "KAGENT_SERVICE_TRUST_FORWARDED_FOR" in security
    assert "unsafe `X-Forwarded-For`" in security
    assert "non-IP" in security
    assert "canonical" in security
    assert "KAGENT_SERVICE_MAX_CONCURRENT_RUNS" in security
    assert "KAGENT_SERVICE_MAX_REQUEST_BYTES" in security
    ...
    assert "KAGENT_SERVICE_REQUEST_TIMEOUT_SECONDS" in security
    assert "request_body_timeout" in security
    assert "KAGENT_SERVICE_ALLOW_FULL_TRACE_RESPONSE" in security
    assert "full_trace_response_must_be_disabled" in security
    assert "KAGENT_SERVICE_PROTECT_DIAGNOSTICS" in security
    ...
    assert "KAGENT_SERVICE_RUNTIME_ALLOWED_TOOLS" in security
    assert "KAGENT_SERVICE_RUNTIME_ALLOWED_TOOLS_BY_SUBJECT" in security
