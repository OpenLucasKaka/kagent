...
def build_runtime_graph(
    *,
    ...
    graph.add_node("prepare_action", _runtime_prepare_action_graph_node)
    graph.add_node("mark_action_executing", _runtime_mark_action_graph_node)
    graph.add_node("execute_action", _runtime_execute_action_graph_node)
...
def _route_after_planner(state: RuntimeGraphState) -> str:
    planner = state.get("initial_planner")
    if not isinstance(planner, dict) or planner.get("status") != "ok":
        return "runtime_loop"
    plan = planner.get("plan")
    actions = plan.get("actions") if isinstance(plan, dict) else None
    return "prepare_action" if isinstance(actions, list) and len(actions) == 1 else "runtime_loop"


def _route_after_prepare_action(state: RuntimeGraphState) -> str:
...
def _route_after_mark_action(state: RuntimeGraphState) -> str:
...
def _runtime_prepare_action_graph_node(
    state: RuntimeGraphState,
    runtime: Any,
) -> RuntimeGraphState:
    started_at = _utc_timestamp()
    ...
    projected_input, input_redacted = _checkpoint_plan_projection(resolved_input)
    projected_action, action_redacted = _checkpoint_plan_projection(action.to_dict())
...
def _runtime_mark_action_graph_node(
    state: RuntimeGraphState,
    runtime: Any,
) -> RuntimeGraphState:
    started_at = _utc_timestamp()
...
def _runtime_execute_action_graph_node(
    state: RuntimeGraphState,
    runtime: Any,
) -> RuntimeGraphState:
    started_at = _utc_timestamp()
    ...
    return {
        ...
        "initial_action_outcome": {
            "status": observation.status,
            "observation": _checkpoint_safe_value(observation.to_dict()),
            "should_replan": (
...
def _failed_initial_action_outcome(
    *,
    ...
    timing = _timing_fields(started_at, started_timer)
    return {
        "status": "failed",
...
def _append_checkpoint_progress_values(
    ...
    progress_events.append(_checkpoint_safe_value(event_with_run_id))
    event_sink = context.get("event_sink")
    if event_sink is not None:
        try:
            event_sink(event_with_run_id)
